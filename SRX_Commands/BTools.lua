...
module.CommandDescription = "Gives the BTool to a player"; -- description of the command
...
module.Execute = function(parameters:table)
	...
	if meetsRequirements then
		...
		if serverUtil.PlayerCanUseCommand(executor,script) then
			...
			if target then
				local btools = ASSETS:WaitForChild("BTools")
				
				btools:Clone().Parent = target.Backpack
				execSuccess = true
			end
		end
	end
