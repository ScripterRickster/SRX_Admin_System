...
module.CommandDescription = "To send a private message to some other player"; -- description of the command
...
module.Execute = function(parameters:table)
	...
	if meetsRequirements then
		...
		if serverUtil.PlayerCanUseCommand(executor,script) then
			...
			if target and parameters["MESSAGE"] then
				
				local nameClr = executor:GetAttribute("SRX_RANKCOLOUR")
				if nameClr == nil or typeof(nameClr) ~= "Color3" then
					nameClr = Color3.fromRGB(255,255,255)
				end
				
				nameClr = nameClr:ToHex()
				
				local msg = serverUtil.FilterMessage(target,parameters["MESSAGE"],true)
				msg = "<b><font color='#"..nameClr.."'>"..executor.Name.."</font></b> sent: <br/>"..msg

				CSC_Event:FireClient(target,"NOTIFICATION","PRIVATE MESSAGE",msg)
