...
module.CommandDescription = "Announces a message to all servers"; -- description of the command
...
module.Execute = function(parameters:table)
	...
	if meetsRequirements then
		...
		if serverUtil.PlayerCanUseCommand(executor,script) then
			if parameters["MESSAGE"]  then
				local filteredMsg = serverUtil.FilterMessage(executor,parameters["MESSAGE"])

				if game.PrivateServerId ~= "" then
					local announce_cmd = require(script.Parent.Announce)
					announce_cmd.Execute(parameters)
				else
					local data = {
						["STAFF_ID"] = executor.UserId;
						["MESSAGE"] = filteredMsg;
					}
