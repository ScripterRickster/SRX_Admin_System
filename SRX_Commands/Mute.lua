...
module.CommandDescription = "Mutes the target if they're in-game"; -- description of the command
		...
		if serverUtil.PlayerCanUseCommand(executor,script) then
			...
			if target  then
				...
				if tRankId < e_rID and not target:GetAttribute("SRX_MUTED") then
					...
					if muteReason == nil then muteReason = "N/A" end
					
					muteReason = serverUtil.FilterMessage(executor,muteReason)
