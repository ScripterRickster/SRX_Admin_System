...
module.CommandDescription = "Stops the player from flying"; -- description of the command
...
module.Execute = function(parameters:table)
	...
	if meetsRequirements then
		...
		if serverUtil.PlayerCanUseCommand(executor,script) then
			...
			if target and target:GetAttribute("SRX_FLYING") then
				...
				if tRankId <= e_rID then
					...
					for _,v in pairs(flyingAssets:GetChildren()) do
						if hrp:FindFirstChild(v.Name) then
							hrp:FindFirstChild(v.Name):Destroy()
						end
					end
					
					CSC_Event:FireClient(target,"unfly")
